...
function parseSource(md) {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    // Smart title detection: try `**H1:** <title>`, then
    // `## CLUSTER X.Y — <title>`, then a real `# <title>` (skipping
    // `# PILLAR N — ...` section markers), then a `**<bold>**` first line.
    // Fallback to `Pillar N` placeholder.
    let title = null;
